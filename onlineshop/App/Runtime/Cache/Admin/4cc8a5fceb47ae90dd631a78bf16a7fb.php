...
<table height="100%" cellspacing=0 cellpadding=0 width=170 background=<?php echo (C("ADMIN_IMG_URL")); ?>/menu_bg.jpg border=0>
    ...
    <div>
        <!-- 面包屑导航 -->
        <div class="crumbs">
            当前位置：商品管理-><a href="/index.php/Admin/Goods/upd">修改商品</a>
        ...
        <div class="tab-div">
            ...
            <div class="tabbody-div">
                <form action="/index.php/Admin/Goods/upd" method="post" id="updFrom" enctype="multipart/form-data">
                    ...
                    <table align="center" width="90%" class="chl">
                        ...
                        <tr>
                            ...
                            <td>
                                <select id=""  name="cat_id">
                                    ...
                                    <?php if(is_array($catinfo)): foreach($catinfo as $key=>$v): if(($v["cat_id"]) == $info["cat_id"]): ?><option value="<?php echo ($v["cat_id"]); ?>" selected>
                                        <?php else: ?>
                        ...
                        <tr>
                            ...
                            <td>
                                <select name="brand_id" >
                                    <option value="0">--请选择--</option>
                                    <?php if(is_array($brandinfo)): foreach($brandinfo as $key=>$v): if(($v["brand_id"]) == $info["brand_id"]): ?><option value="<?php echo ($v["brand_id"]); ?>" selected>
                                                <?php else: ?>
                                            <option value="<?php echo ($v["brand_id"]); ?>"><?php endif; ?>
                                        <?php echo ($v["brand_name"]); ?></option><?php endforeach; endif; ?>
                                </select>
                            </td>
                        </tr>
                        <tr>
                        ...
                        <tr>
                            ...
                            <td>
                                <input type="text" name="promote_price" value="<?php echo ($info["promote_price"]); ?>" disabled="disabled" class="disable">
                            </td>
                        </tr>
                        <tr>
                    ...
                    <table align="center" width="90%" class="chl" style="display: none">
                        <tr>
                            ...
                            <td>
                                <textarea name="goods_desc" id="goods_desc" style="width: 700px; height: 150px;"><?php echo ($info["goods_desc"]); ?></textarea>
                            </td>
                        </tr>
                        <tr>
                            <td align="right" class="label">Package included：</td>
                            <td>
                                <textarea name="goods_package"  style="width: 700px; height: 100px;"><?php echo ($info["goods_package"]); ?></textarea>
                            </td>
                        </tr>
                    </table>
                    <!-- 其他信息 -->
                    <table align="center" width="90%" class="chl" style="display: none">
                        <tr>
                            ...
                            <td><input type="text" name="goods_sn" value="<?php echo ($info["goods_sn"]); ?>"></td>
                        </tr>
                        <tr>
                            <td align="right" class="label">商品重量：</td>
                            <td><input type="text" name="goods_weight" value="<?php echo ($info["goods_weight"]); ?>">
                                <select name="weight_unit">
                                    <?php if(($info["weight_unit"]) == "0"): ?><option value="0" selected>千克</option>
                                        <option value="1">克</option>
                                    <?php else: ?>
                                        <option value="0">千克</option>
                                        <option value="1" selected>克</option><?php endif; ?>
                        ...
                        <tr>
                            ...
                            <td><input type="text" name="warning_number" value="<?php echo ($info["warning_number"]); ?>"></td>
                        </tr>
                        <tr>
                            <td align="right" class="label">商品链接：</td>
                            <td><input type="text" name="goods_link" value="<?php echo ($info["goods_link"]); ?>"></td>
                        </tr>
                        <tr>
                            <td align="right" class="label">加入推荐：</td>
                            <td>
                                <?php if(($info["is_best"]) == "0"): ?><input type="checkbox" name="is_best">精品
                                <?php else: ?>
                                    <input type="checkbox" name="is_best" checked>精品<?php endif; ?>
                                <?php if(($info["is_new"]) == "0"): ?><input type="checkbox" name="is_new">新品
                                <?php else: ?>
                                    <input type="checkbox" name="is_new" checked>新品<?php endif; ?>
                                <?php if(($info["is_hot"]) == "0"): ?><input type="checkbox" name="is_hot">热销
                                <?php else: ?>
                                    <input type="checkbox" name="is_hot" checked>热销<?php endif; ?>
                            </td>
                        </tr>
                        <tr>
                            <td align="right" class="label">上架：</td>
                            <td>
                                <?php if(($info["is_onsale"]) == "0"): ?><input type="checkbox"  name="is_onsale">
                                <?php else: ?>
                                    <input type="checkbox" checked="checked" name="is_onsale"><?php endif; ?>
                                 打勾表示允许销售，否则不允许销售
                            </td>
                        </tr>
                        <tr>
                            ...
                            <td>
                                <textarea name="goods_brief" cols="40" rows="5" ><?php echo ($info["goods_brief"]); ?></textarea>
                            </td>
                        </tr>
                    </table>
                    <!-- 商品属性 -->
                    <table align="center" width="90%" class="chl" id="goodsAttrUpd" style="display: none">
                        <tr>
                            <td align="right" class="label">商品类型：</td>
                            <td>
                                <select name="type_id" class="goods_type_upd">
                                    <option value="0">请选择商品类型</option>
                                    <?php if(is_array($typeinfo)): foreach($typeinfo as $key=>$v): if(($v["type_id"]) == $info["type_id"]): ?><option value="<?php echo ($v["type_id"]); ?>" selected><?php echo ($v["type_name"]); ?></option>
                                        <?php else: ?>
